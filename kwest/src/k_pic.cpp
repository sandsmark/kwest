...
bool unix_init_pictures (void)
{
  ...
  int namelen;
/*
  if ((filename = (char*)malloc(2 * strlen(story_name) + 10)) == NULL)
    return FALSE;
...
/*
  ...
  if (!acs_initialized) {
    plus     = unix_plain_ascii ? '+'  : ACS_PLUS;
    ltee     = unix_plain_ascii ? '<'  : ACS_LTEE;
    rtee     = unix_plain_ascii ? '>'  : ACS_RTEE;
    ttee     = unix_plain_ascii ? '^'  : ACS_TTEE;
    btee     = unix_plain_ascii ? 'v'  : ACS_BTEE;
    hline    = unix_plain_ascii ? '-'  : ACS_HLINE;
    vline    = unix_plain_ascii ? '|'  : ACS_VLINE;
    ckboard  = unix_plain_ascii ? ':'  : ACS_CKBOARD;
    urcorner = unix_plain_ascii ? '\\' : ACS_URCORNER;
    ulcorner = unix_plain_ascii ? '/'  : ACS_ULCORNER;
    llcorner = unix_plain_ascii ? '\\' : ACS_LLCORNER;
    lrcorner = unix_plain_ascii ? '/'  : ACS_LRCORNER;
